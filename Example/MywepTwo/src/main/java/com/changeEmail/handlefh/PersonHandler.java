package com.changeEmail.handlefh;
...
import com.changeEmail.entity.Person;
...
public class PersonHandler 
{
	...
	public ServerResponse getPerson(ServerRequest request)
	{
		...
		return people.stream().filter(p -> p.getId().equals(personid)).findFirst().map(person ->ServerResponse.ok()
				.contentType(MediaType.APPLICATION_JSON).body(person)).orElse(ServerResponse.notFound().build());
